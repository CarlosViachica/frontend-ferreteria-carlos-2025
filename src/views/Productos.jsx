...
const Productos = () => {
  ...
  const agregarProducto = async () => {
    ...
    try {
      const respuesta = await fetch("http://localhost:3002/api/registrarproducto", {
        method: "POST",
  ...
  const guardarEdicion = async () => {
    ...
    try {
      const respuesta = await fetch(
        `http://localhost:3002/api/actualizarproductoPatch/${productoEditado.id_producto}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  const confirmarEliminacion = async () => {
    try {
      const respuesta = await fetch(
        `http://localhost:3002/api/eliminarproducto/${productoAEliminar.id_producto}`,
        {
          method: "DELETE",
        }
      );
