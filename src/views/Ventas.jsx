...
const Ventas = () => {
  ...
  const obtenerNombreCliente = async (idCliente) => {
    if (!idCliente) return '—';
    try {
      const resp = await fetch(`http://localhost:3002/api/cliente/${idCliente}`);
      if (!resp.ok) return '—';
  ...
  const obtenerVentas = async () => {
    try {
      const resp = await fetch('http://localhost:3002/api/ventas');
  ...
  const obtenerDetallesVenta = async (id_venta) => {
    try {
      const resp = await fetch('http://localhost:3002/api/detallesventas');
  ...
  const obtenerClientes = async () => {
    try {
      const resp = await fetch('http://localhost:3002/api/clientes');
  ...
  const agregarVenta = async () => {
    ...
    try {
      const ventaResp = await fetch('http://localhost:3002/api/registrarventa', {
        method: 'POST',
      ...
      for (const d of detallesNuevos) {
        await fetch('http://localhost:3002/api/registrardetalleventa', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const actualizarVenta = async () => {
    const total = detallesNuevos.reduce((sum, d) => sum + (d.cantidad * d.precio_unitario), 0);
    try {
      await fetch(`http://localhost:3002/api/actualizarVentaPatch/${ventaAEditar.id_venta}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      for (const d of actuales) {
        await fetch(`http://localhost:3002/api/eliminardetalleventa/${d.id_detalle_venta}`, { method: 'DELETE' });
      }
  ...
  const eliminarVenta = async () => {
    try {
      await fetch(`http://localhost:3002/api/eliminarventa/${ventaAEliminar.id_venta}`, { method: 'DELETE' });
      await obtenerVentas();
