...
const Compras = () => {
  ...
  const obtenerNombreEmpleado = async (idEmpleado) => {
    ...
    try {
      const resp = await fetch(`http://localhost:3002/api/empleado/${idEmpleado}`);
      if (!resp.ok) return '—';
  ...
  const obtenerNombreProducto = async (idProducto) => {
    ...
    try {
      const resp = await fetch(`http://localhost:3002/api/producto/${idProducto}`);
      if (!resp.ok) return '—';
      const data = await resp.json();
  ...
  const obtenerCompras = async () => {
    try {
      const resp = await fetch('http://localhost:3002/api/compras');
  ...
  const obtenerDetallesCompra = async (id_compra) => {
    try {
      const resp = await fetch('http://localhost:3002/api/detallescompras');
  ...
  const obtenerEmpleados = async () => {
    try {
      const resp = await fetch('http://localhost:3002/api/empleados');
  ...
  const agregarCompra = async () => {
    ...
    try {
      const resp = await fetch('http://localhost:3002/api/registrarCompra', {
        method: 'POST',
    ...
    body: JSON.stringify({ ...nuevaCompra, total_compra: total })
      ...
      for (const d of detallesNuevos) {
        await fetch('http://localhost:3002/api/registrarDetalleCompra', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const actualizarCompra = async () => {
    ...
    try {
      // actualizar compra
      await fetch(`http://localhost:3002/api/actualizarCompraPatch/${compraAEditar.id_compra}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      const respDetalles = await fetch('http://localhost:3002/api/detallescompras');
      const todos = await respDetalles.json();
      const actuales = todos.filter(d => d.id_compra === compraAEditar.id_compra);
      for (const a of actuales) {
        await fetch(`http://localhost:3002/api/eliminardetalles_compras/${a.id_detalle_compra}`, { method: 'DELETE' });
      }

      for (const d of detallesNuevos) {
        await fetch('http://localhost:3002/api/detallescompras', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const eliminarCompra = async () => {
    try {
      await fetch(`http://localhost:3002/api/compras/${compraAEliminar.id_compra}`, { method: 'DELETE' });
      await obtenerCompras();
